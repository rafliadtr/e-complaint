	...
	<?php 
		...
		ON complain.id_user=akun_user.id_user WHERE complain.status='proses' 
		ORDER BY complain.no_antrian ASC");
		...
		<tr>
			...
			<td><a class="btn modal-trigger blue" href="#more?id_pengaduan=<?php 
			echo $r['no_antrian'] ?>">More</a>  
			<a class="btn red" onclick="return confirm('Anda Yakin Ingin Menghapus?')" 
			href="index.php?p=pengaduan_hapus&no_antrian=<?php echo $r['no_antrian'] ?>">
			Hapus</a></td>
          ...
          <div class="modal-content">
			...
			<?php 
				if(isset($_POST['tanggapi'])){
					$tgl = date('Y-m-d');
					$query = mysqli_query($koneksi,"INSERT INTO tindak_lanjut VALUES 
					(NULL,'".$r['no_antrian']."','".$_SESSION['data']['id_admin']."',
					'".$_POST['tanggapan']."','".$tgl."')");
					if($query){
